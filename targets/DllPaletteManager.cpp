...
// or,,no, via complete luck, it actually requires it, for globals
...
//extern PaletteManager* palManPtr;
...
namespace AsmHacks
{
    ...
    void palettePatcherCallback() {
        ...
        if(!didIt) {
            didIt = true;
            /*if(palManPtr != NULL) {
                palManPtr->loadOurData();
            } else {
                netlog("palManPtr NULL");
            }*/

            netlog("idk: %d", netManPtr->config.paletteManager.loadOurDataInit);
        }
